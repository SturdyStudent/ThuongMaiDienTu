...
exports.userItemId = [
    function (req, res) {
        ...
        try {
            ...
            waitPool().then((result) => {
                return apiResponse.successResponseWithData(res, "Lấy id khách hàng thành công", result.recordsets[0]);
            }).catch(err => { return apiResponse.ErrorResponse(res, err) });
        } catch (err) {
            return apiResponse.ErrorResponse(res, err);
        }
    ...
    function (req, res) {
        ...
        try {
            const waitPool = async () => {
                let pool = await sql.connect(config);
                user = await pool.request()
                    .input('TenKhachHang', sql.NVarChar(50), req.body.TenKhachHang)
                    .execute('SelectTenKhachHang');
                return voucher;
            }
            waitPool().then((result) => {
