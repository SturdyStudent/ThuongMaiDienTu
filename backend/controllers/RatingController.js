...
function PublisherData(data) {
    ...
    this.publisherImage = data.publisherImage;
}
        ...
        try {
            Publisher.findById(req.params.id, function (err, foundPublisher) {
                if (foundPublisher === null) {
                    ...
                    if (foundPublisher.user.toString() !== req.user._id) {
                        return apiResponse.unauthorizedResponse(res, "Bạn không được cấp quyền thực hiện hành động này.");
                    } else {
                        //delete Publisher.
        ...
        try {
            ...
            if (!errors.isEmpty()) {
                ...
                if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
                    ...
                    Publisher.findById(req.params.id, function (err, foundPublisher) {
                        if (foundPublisher === null) {
                            return apiResponse.notFoundResponse(res, "Publisher not exists with this id");
                        } else {
