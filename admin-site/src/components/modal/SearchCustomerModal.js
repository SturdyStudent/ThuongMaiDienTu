...
export default function SearchModal({ changeSelectedUser, callbackSetOpenModal }) {
    ...
    var results = _.map(userList, function (o) {
        if (String(o.HoTen).toLowerCase().includes(searchTerm.toLowerCase())) return o;
    })
            ...
            <div className='container-out-notification row' onClick={handleCloseSuccessModal}>
                <div className='container-notification offset-4 col-md-4 p-4' style={{ "top": "12vh" }} onClick={preventPropagation}>
                    ...
                    <div className="input-group mb-3">
                        <input type="text" className="form-control" onChange={e => handleChangeSearchTerm(e)} placeholder="Nhập tên khách hàng" aria-label="Recipient's username" aria-describedby="basic-addon2" />
                    ...
                    <Element name="test7" className="element" id="containerElement" style={{
                    ...
                    }}>
                        {
                            results.map((item) => {
                                return (
                                    <div key={item.MaKH} className=" pt-4 border-bottom pb-3">
                                        <div className='row col-md-12'>
                                            <div className='col-md-3 d-flex justify-content-center'>
                                                <button onClick={() => { changeSelectedUser(item); handleCloseSuccessModal(); }} type='button' style={{ maxHeigh: "40px", width: "70px" }} className='btn btn-danger text-white'>Chọn</button>
                                            </div>
